 ...
 * 计算从数字A到数字B的转移概率
 */
function buildMarkovTransitionMatrix(
  draws: LotteryDraw[], 
 ...
 */
function calculateMissingWeights(
  draws: LotteryDraw[],
  position: 'hundred' | 'ten' | 'one'
): Map<number, number> {
 ...
 * 检测数字出现的周期性规律
 */
function detectPeriodicPattern(
  draws: LotteryDraw[],
  position: 'hundred' | 'ten' | 'one',
  digit: number
): number {
 ...
 * 分析数字是否有连续递增/递减趋势
 */
function analyzeTrendPattern(draws: LotteryDraw[], position: 'hundred' | 'ten' | 'one'): Map<number, number> {
  const weights = new Map<number, number>();
  
  if (draws.length < 3) {
    for (let i = 0; i <= 9; i++) weights.set(i, 0);
    return weights;
  }
...
export function generatePrediction(
  ...
  const streakNextDigits = new Map<number, number>();
