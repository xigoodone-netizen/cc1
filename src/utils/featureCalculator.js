 ...
 * 使用动态阈值和标准差计算
 */
export function calculateHotCold(draws, windowSize = 30) {
 ...
 * 增加历史最长连中/连挂记录
 */
export function calculateStreaks(draws) {
    const stats = [];
 ...
 * 3. 计算和值
 */
export function calculateSumValue(draw) {
    return draw.hundred + draw.ten + draw.one;
}
/**
 * 4. 计算跨度
 */
export function calculateSpan(draw) {
 ...
 * 5. 大小比 (5-9 为大)
 */
export function calculateSizeRatio(draw) {
    const digits = [draw.hundred, draw.ten, draw.one];
 ...
 * 6. 单双比
 */
export function calculateOddEvenRatio(draw) {
    const digits = [draw.hundred, draw.ten, draw.one];
 ...
 * 7. 质合比
 */
export function calculatePrimeCompositeRatio(draw) {
    const digits = [draw.hundred, draw.ten, draw.one];
 ...
 * 8. 012路比
 */
export function calculateRoad012Ratio(draw) {
    const digits = [draw.hundred, draw.ten, draw.one];
 ...
 * 9. 形态分析 (组三、组六、豹子)
 */
export function calculatePattern(draw) {
 ...
 * 10. AC值计算
 */
export function calculateACValue(draw) {
    const digits = [draw.hundred, draw.ten, draw.one];
 ...
 * 计算位置遗漏
 */
export function calculateMissing(draws, position, digit) {
    let missing = 0;
    for (let i = draws.length - 1; i >= 0; i--) {
 ...
 * 计算单个位置的详细统计 - 优化版
 */
export function calculatePositionStats(draws, position, windowSize = 30) {
    const recentDraws = draws.slice(-windowSize);
...
// ==================== 完整特征集计算 ====================
export function calculateAllFeatures(draws, windowSize = 30) {
    if (draws.length === 0)
        return null;
 ...
 * 位置差计算
 */
export function calculatePositionDiff(draw) {
    return {
 ...
 * 重号位置分析
 */
export function calculateRepeatPosition(current, previous) {
    return [
 ...
 * 邻号位置分析
 */
export function calculateNeighborPosition(current, previous) {
    return [
 ...
 * 斜连号模式
 */
export function calculateDiagonalPattern(_draws) {
    return "无";
}
/**
 * 对称号模式
 */
export function calculateSymmetryPattern(draw) {
    if (draw.hundred === draw.one)
        return '百个对称';
    if (draw.hundred === draw.ten)
        return '百十对称';
    if (draw.ten === draw.one)
        return '十个对称';
    return '无对称';
}
export function calculateSumTail(draw) {
    return calculateSumValue(draw) % 10;
}
export function calculateSpanTail(draw) {
    return calculateSpan(draw) % 10;
}
export function calculateSumSpanCombo(draw) {
    return `${calculateSumValue(draw)}-${calculateSpan(draw)}`;
}
export function calculateSizeOddEvenCombo(draw) {
    const size = calculateSizeRatio(draw);
    const oe = calculateOddEvenRatio(draw);
    return `大${size.big}小${size.small}奇${oe.odd}偶${oe.even}`;
}
export function calculatePrimeRoadCombo(draw) {
...
export function calculateHotWarmColdZone(draws, windowSize = 30) {
...
export function calculateMissingLayers(draws) {
...
export function calculateDigitSum(draw) {
    return calculateSumValue(draw);
}
export function calculateProductTail(draw) {
